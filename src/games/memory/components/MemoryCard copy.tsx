...
type Props = {
  ...
  onPress: () => void;
  isDisabled: boolean;
  cardSize?: number;  // ← Nouvelle prop (défaut si pas passé)
};

const MemoryCard = ({ icon, isFlipped, isMatched, onPress, isDisabled, cardSize = 65 }: Props) => {
    ...
    >
      <MotiView
        style={[
          ...
          { 
            ...
            // margin: Math.max(2, cardSize * 0.05),
          },
          {
      ...
      >
        ...
        >
          ...
          <MaterialCommunityIcons
            name="help"
            size={cardSize * 0.6}  // ← DYNAMIQUE : Scale avec la carte (ex. 42px sur 70)
            color={theme.card} // Assurez-vous que theme.card a un bon contraste avec theme.primary
          />
        </MotiView>
        ...
        >
          ...
          <MaterialCommunityIcons
            name={icon as any}
            size={cardSize * 0.6}  // ← DYNAMIQUE : Même scale
            color={isMatched ? theme.card : theme.primary} // Amélioration du contraste
          />
        </MotiView>
      </MotiView>
