...
const SplashScreen: FC = () => {
    ...
    //         // ⭐ IMPORTANT : Attendre que le splash natif soit visible
    ...
    const initialize = async () => {
      try {
        // 1. Cache le splash natif d'Expo immédiatement
        await SplashScreen.hideAsync();
        
        // 2. Anime notre splash personnalisé
        if (mounted) {
          Animated.parallel([
            Animated.timing(fadeAnim, {
              toValue: 1,
        ...
        // 3. Attendre 2 secondes puis naviguer
        setTimeout(() => {
          if (mounted) {
            Animated.timing(fadeAnim, {
              toValue: 0,
